...
const skillNames=bank=>bank.level.startsWith('3')?['Analyser le fonctionnement d’un objet technique','Diagnostiquer et proposer une amélioration']:['Relier besoins, fonctions et solutions techniques','Exploiter des données pour justifier un choix'];
const classKey=value=>norm(value).replace(/\s+/g,'');
const compareAttempts=(a,b)=>classKey(a.classe).localeCompare(classKey(b.classe),'fr',{numeric:true})||a.students[0].first.localeCompare(b.students[0].first,'fr');
...
function grade(a,bank){const scores=[0,0];for(const q of a.questions)if(a.answers[q.id]===q.correct){const i=bank.level.startsWith('3')?Math.floor(q.skill/2):(q.skill===0||q.skill===2?0:1);scores[i]++;}return {total:scores[0]+scores[1],skills:scores.map((score,i)=>({name:skillNames(bank)[i],score,max:10,level:competence(score)}))};}
...
export class EvaluationStore extends DurableObject{
 ...
 async route(req){const url=new URL(req.url),path=url.pathname.replace('/api/evaluations','');
  ...
  if(req.method==='GET'&&path.startsWith('/teacher')){this.teacher(req);const banks=this.getBanks();
   if(path==='/teacher/export'){const ids=url.searchParams.get('session');const sessions=ids?[this.session(ids)]:this.sql.exec('SELECT data FROM sessions ORDER BY created').toArray().map(r=>JSON.parse(r.data));const classe=url.searchParams.get('classe');const lines=[['Évaluation','Séance','Classe','Prénom','Initiale du nom','Note /20','Statut','Compétence 1','Score C1 /10','Niveau C1','Compétence 2','Score C2 /10','Niveau C2','Copie','Début UTC','Rendu UTC','Saisie dans Pronote']];for(const s of sessions)for(const a of this.listAttempts(s.id).filter(a=>!classe||classKey(a.classe)===classKey(classe)).sort(compareAttempts)){const g=a.finished?grade(a,banks[s.assessment]):null;for(const [studentIndex,p] of a.students.entries())lines.push([banks[s.assessment].title,s.label,a.classe,p.first,p.initial,g?.total??'',a.finished?'Rendue':'En cours',...skillNames(banks[s.assessment]).flatMap((name,i)=>[name,g?.skills[i].score??'',g?.skills[i].level??'']),a.id,new Date(a.started).toISOString(),a.finished?new Date(a.finished).toISOString():'',a.finished?(a.pronote?.[studentIndex]?'Oui':'Non'):'']);}return new Response('\uFEFF'+[lines[0],...lines.slice(1).sort((a,b)=>classKey(a[2]).localeCompare(classKey(b[2]),'fr',{numeric:true})||a[3].localeCompare(b[3],'fr'))].map(r=>r.map(csvCell).join(';')).join('\r\n'),{headers:{'Content-Type':'text/csv; charset=utf-8','Content-Disposition':`attachment; filename="notes-technologie${ids?'-'+this.session(ids).assessment:''}.csv"`,'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}});}
   ...
   const id=url.searchParams.get('session');if(id){const s=this.session(id);return json({session:this.summary(s,banks),attempts:this.listAttempts(id).map(a=>({id:a.id,students:a.students,classe:a.classe,started:a.started,deadline:a.deadline,finished:a.finished,answered:Object.keys(a.answers).length,blur:a.blur,pronote:a.pronote||{},result:a.finished?grade(a,banks[s.assessment]):null}))});}
  ...
  if(path.startsWith('/teacher')){this.teacher(req);const banks=this.getBanks();
   ...
   if(path==='/teacher/pronote'){const r=this.one('SELECT data FROM attempts WHERE id=?',field(b.id));if(!r)fail(404,'Copie introuvable.');const a=this.expire(JSON.parse(r.data));if(!a.finished)fail(409,'La copie doit être rendue avant de reporter la note.');if(!Number.isInteger(b.student)||b.student<0||b.student>=a.students.length||typeof b.entered!=='boolean')fail(400,'Élève ou statut non valide.');a.pronote={...a.pronote,[b.student]:b.entered};this.putAttempt(a);return json({ok:true});}
   if(path==='/teacher/delete'){if(b.confirm!==true)fail(400,'Confirmez la suppression de la copie.');const r=this.one('SELECT id FROM attempts WHERE id=?',field(b.id));if(!r)fail(404,'Copie introuvable.');this.ctx.storage.transactionSync(()=>{this.sql.exec('DELETE FROM identities WHERE attempt_id=?',b.id);this.sql.exec('DELETE FROM attempts WHERE id=?',b.id);});return json({ok:true});}
